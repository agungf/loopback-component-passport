...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  switch (authType) {
    ...
    case 'openid connect':
      ...
      break;
    default:
      var authStrategy = new AuthStrategy(_.defaults({
          clientID: clientID,
          clientSecret: clientSecret,
          callbackURL: callbackURL,
          passReqToCallback: true
        ...
        function (req, accessToken, refreshToken, profile, done) {
          //save the refresh token to loopback app instance
          self.app.set('refreshToken', refreshToken);
      ...
      passport.use(name, authStrategy);
      refresh.use(name, authStrategy);
  }
...
PassportConfigurator.prototype.refreshToken = function (refreshToken, cb) {
  var self = this;
  refresh.requestNewAccessToken('google-link', refreshToken, function(err, accessToken) {
      cb(accessToken);
  });
};